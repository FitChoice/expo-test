...
const buildMonth = (date: Date, calendarByDate: Map<string, CalendarItem>) => {
	...
	const firstDayOffset =
		(new Date(date.getFullYear(), date.getMonth(), 1).getDay() + 6) % 7 // convert Sunday=0 to Monday=0
...
export const CalendarStatistic = () => {
	...
	const [calendarByDate, setCalendarByDate] = useState<Map<string, CalendarItem>>(
		new Map()
	)
	...
	if (isLoading) {
		return (
			<View className="bg-brand-dark-400 mb-6 items-center justify-center rounded-3xl p-6">
				<ActivityIndicator color={ACTIVE_COLOR} />
				<Text className="text-body-medium mt-3 text-center text-light-text-200">
					Загружаем календарь...
				</Text>
			</View>
		)
	...
	if (error) {
		return (
			<View className="bg-brand-dark-400 mb-6 rounded-3xl p-6">
			...
			{months.map(({ title, days, key }) => (
				<View key={key} className="mb-6">
					...
					<View className="flex-row flex-wrap">
						{days.map((day, idx) => (
							<Day key={`${key}-${idx}`} {...day} onPress={() => handleDayPress(day)} />
						))}
					</View>
				</View>
			))}
