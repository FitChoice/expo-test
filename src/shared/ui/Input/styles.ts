import { type InputVariant, type InputSize } from './types'
...
export const getInputStyles = (
	...
	let labelClasses = 'text-sm mb-3 text-white ' // t3 regular, Light text/100
	const errorClasses = 'text-xs mt-2 text-feedback-negative-900 ' // t4
	const helperClasses = 'text-xs mt-2 text-light-text-200 ' // t4

	// Размеры
	if (size === 'settings') {
		// Settings field/dropdown - 8px border radius, меньше padding
		inputClasses += 'rounded-lg px-2 py-2 min-h-[36px] sm:min-h-[40px] ' // 8px
	} else {
		// Default - 16px border radius
		inputClasses +=
			'rounded-2xl px-3 py-2.5 min-h-[44px] sm:px-4 sm:py-3 sm:min-h-[48px] ' // 16px, адаптивный padding
	}
	...
	if (size === 'default') {
		if (disabled) {
			// Disabled state
			inputClasses += 'bg-fill-800 opacity-50 '
			...
			// Focused state
			inputClasses +=
				'bg-fill-800 border border-brand-purple-500 shadow-[0px_0px_5px_0px_rgba(187,162,254,0.8)] '
		} else {
			// Default state
		...
		if (disabled) {
			...
			inputClasses +=
				'border border-brand-purple-900 shadow-[0px_0px_5px_0px_rgba(187,162,254,1)] '
		} else {
...
export const getTextareaStyles = (
	...
	if (disabled) {
		...
		classes +=
			'border border-brand-purple-900 shadow-[0px_2px_5px_0px_rgba(187,162,254,0.8)] '
	} else {
...
export const getTextColor = (
	...
	if (disabled) {
		return '#949494' // Light text/500
	}
	if (filled || isFocused) {
		return '#FFFFFF' // Light text/100
	}
	return '#949494' // Light text/500 для placeholder
}
