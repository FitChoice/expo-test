...
- **`partial_rom_threshold_down`** (число, 0–1, опционально): порог для детекции неполной амплитуды на нижней фазе (Down→Bottom).
  - Если пользователь достиг N% глубины от `down_enter` до `bottom_enter`, но не перешёл в `Bottom`, генерируется событие `PARTIAL_ROM (down)`
  - Пример: при пороге 0.75 и `down_enter=30`, `bottom_enter=17`, если дошёл до 20 (77%), но не до 17 — неполная амплитуда
- **`partial_rom_threshold_up`** (число, 0–1, опционально): порог для детекции неполной амплитуды на верхней фазе (Up→Top).
  - Если пользователь достиг N% высоты от `up_enter` до `top_enter`, но не перешёл в `Top`, генерируется событие `PARTIAL_ROM (up)`
  - Полезно для упражнений, где важно полностью выпрямиться (ягодичный мост, отжимания)
...
- **Axis**: `quadruped_hip_extension_progress_l/r` (`180° - angle(knee-hip-shoulder)`, растёт при подъёме бедра)
- **Пороги**: прогресс увеличивается при подъёме ноги назад (≈6 → 32)
