...
4.  **Выполнение** (`execution`):
    ...
    *   Детекция фаз движения (Top -> Down -> Bottom -> Up) и инкрементальный подсчет повторов.
...
5.  **Смена стороны и отдых** (`rest` / `side_switch`):
    *   **Между сторонами (mirror):** Для упражнений с `is_mirror: true` после первой стороны (обычно правой) запускается короткий отдых (`restType: 'rep'`, 5 сек). Если отдых > 10 сек, может быть показано видео с коррекцией техники (`restPhase: 'practice'`). После отдыха следует этап `side_switch` для смены рабочей стороны.
    *   **Между подходами:** После выполнения всех сторон текущего подхода запускается отдых (`restType: 'set'`) длительностью `rest_between_sets`. После него возвращаемся на этап `position`.
    *   **Между упражнениями:** После завершения всех подходов запускается отдых (`restType: 'exercise'`) длительностью `rest_after_exercise`, затем переход к следующему упражнению.
6.  **Синхронизация и завершение**:
    *   **Отправка упражнения:** После завершения всех подходов упражнения данные (ID, общее кол-во повторов, качество) отправляются на бэк через `onExecuteExercise`.
    *   **Завершение тренировки:** После последнего упражнения вызывается `onCompleteTraining` с итоговой аналитикой за всю сессию:
        *   `report_active_time`: Чистое время выполнения упражнений.
        *   `report_cals`: Сожженные калории (базовый расчет: 0.5 кал/повтор).
        *   `report_duration`: Общая длительность сессии.
        *   `report_technique_quality`: Среднее качество техники по всем подходам.
    *   Состояние тренировки в сторе переходит в `finished`.
