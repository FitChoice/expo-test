...
## 📸 Фото‑прогресс (текущее поведение)
...
- Метаданные: `index.json` в корне пользователя; при чтении битые ссылки удаляются.
- `saveProgressBatch` (по умолчанию `replaceStrategy=last-per-side`) перемещает временные снимки, при необходимости создаёт `MediaLibrary` asset, сохраняет запись в meta и инвалидирует кэш.
...
- `ExistingPhotosScreen`: считает 30-дневный интервал между сериями, показывает оставшиеся дни до следующего окна съёмки, выводит сетку 4 ракурсов в порядке `front → left → right → back`. Кнопка «Переснять» очищает хранилище и запускает съёмку.

### Съёмка (`features/progress-capture/ProgressCaptureFlow`)
Порядок ракурсов в цикле: `front → back → left → right`. Ориентация принудительно портретная (`useOrientation`).
1. **CameraPermission** — заявляет, что фото остаются на устройстве; кнопка «Далее» активна только после выдачи разрешения камеры. Закрытие ведёт на `/stats`.
2. **PhonePosition** — показывает превью с фронтальной камеры, слушает `expo-screen-orientation`; «Далее» доступно только в портретной ориентации.
3. **PositionReady** — `PoseCamera` (MoveNet) с сеткой и силуэтом; после детекта всех 17 keypoints 2 секунды показывается «Начнём!» и шаг автоматически переключается на съёмку.
4. **CountdownCapture** — 5‑секундный отсчёт, фронтальная камера (`CameraView`), `StepProgress` по сторонам; снимает кадр и передаёт во временное состояние.
5. **PreviewScreen** — превью снятого ракурса, `StepProgress`, кнопки «Переснять» (возврат к позиции) и «Далее» (фиксирует снимок и идёт к следующей стороне).
6. **FinalScreen** — миниатюры всех сторон, возможность перезаписать любой ракурс, тумблер «Сохранить в галерею» (запрашивает `MediaLibrary` при первом включении), кнопка «Сохранить» активна только при 4 снимках; `onSave` вызывает `useSaveProgressBatchMutation` и возвращает в вызывающий экран.
